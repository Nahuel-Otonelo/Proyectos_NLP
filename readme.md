...
## 🛠️ Desafíos y Metodología (Desafío 1)
...
1.  Se comparó `MultinomialNB` vs. `ComplementNB`, identificando a `ComplementNB` como el modelo superior.
2.  Se implementó un **`Pipeline`** de `scikit-learn` para encadenar el `TfidfVectorizer` y el `ComplementNB`.
3.  Se utilizó **`GridSearchCV`** para encontrar la mejor combinación de hiperparámetros.
...
* Cada fila se reinterpretó como un **vector de palabra** (un embedding simple).
* Se analizó la similaridad coseno de 5 palabras (`god`, `car`, `president`, etc.) para estudiar las relaciones semánticas y de co-ocurrencia.
